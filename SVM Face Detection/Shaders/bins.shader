            ...
            */
            ...
            float4 frag (v2f ps) : SV_Target
            {
                ...
                if (px.y < 48) {
                    if (px.x >= 192) discard;
                    // 160x90, stride 4
                    uint2 targetPx = uint2(250, 50);
                    targetPx += floor(px / 8) * 4 + (px % 8) * 8;
                    col = bin(targetPx);
                }
                else if (px.y < 144) {
                    if (px.x >= 248) discard;
                    // 250x140, stride 6
                    px.y -= 48;
                    uint2 targetPx = floor(px / 8) * 6 + (px % 8) * 8;
                    col = bin(targetPx);
                }
                else {
                    // 390x220, stride 8
                    px.y -= 144;
                    uint2 targetPx = uint2(0, 140);
                    targetPx += floor(px / 8) * 8 + (px % 8) * 8;
                    col = bin(targetPx);
                }
