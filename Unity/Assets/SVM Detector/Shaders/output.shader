    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f ps) : SV_Target
            {
                ...
                // float d1 = sdBox(p - float2(0.0667, 0.0667), float2(0.0667, 0.0667));
                // float d2 = sdBox(p - float2(0.1333, 0.1333), float2(0.1333, 0.1333));
                // float d3 = sdBox(p - float2(0.2667, 0.2667), float2(0.2667, 0.2667));
                // col.rgb = lerp(col.rgb, float3(0, 1, 0), 1.0-smoothstep(0.0,0.01,abs(d1)));
                // col.rgb = lerp(col.rgb, float3(0, 1, 0), 1.0-smoothstep(0.0,0.01,abs(d2)));
                // col.rgb = lerp(col.rgb, float3(0, 1, 0), 1.0-smoothstep(0.0,0.01,abs(d3)));
                ...
                for (uint i = 0; i < 27; i++) {
                    for (uint j = 0; j < 14; j++) {
                        ...
                        float2 uv2 = (float2(i, j) / float2(27, 14)) * float2(.9, 0.8) +
                            float2(0.07, 0.12);
                        float2 dist = p - uv2;
                        if (cl > 0.4 && abs(dist.x) < 0.06 && abs(dist.y) < 0.10667)
                        {
                ...
                for (uint i = 0; i < 17; i++) {
                    for (uint j = 0; j < 8; j++) {
                        ...
                        float2 uv2 = (float2(i, j) / float2(17, 8)) * float2(0.87, 0.7) + 
                            float2(0.14, 0.24);
                        float2 dist = p - uv2;
                        if (cl > 0.4 && abs(dist.x) < 0.12 && abs(dist.y) < 0.2133)
                        {
                            col.rgb *= float3(2, .5, .5);
                        }
                    }
                ...
                for (uint i = 0; i < 8; i++) {
                    ...
                    float2 dist = p - uv2;
                    if (cl > 0.4 && abs(dist.x) < 0.24 && abs(dist.y) < 0.42667)
                    {
                        col.rgb *= float3(2, .5, .5);
                    }
                }
